EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L MCU_Module:Arduino_Nano_v3.x A1
U 1 1 64FF5C7A
P 3550 3200
F 0 "A1" H 3550 2111 50  0000 C CNN
F 1 "Arduino_Nano_v3.x" H 3550 2020 50  0000 C CNN
F 2 "Module:Arduino_Nano" H 3550 3200 50  0001 C CIN
F 3 "http://www.mouser.com/pdfdocs/Gravitech_Arduino_Nano3_0.pdf" H 3550 3200 50  0001 C CNN
	1    3550 3200
	1    0    0    -1  
$EndComp
$Comp
L Display_Character:HY1602E DS2
U 1 1 64FF96AD
P 6650 3800
F 0 "DS2" H 6650 4781 50  0000 C CNN
F 1 "HY1602E" H 6650 4690 50  0000 C CNN
F 2 "Display:HY1602E" H 6650 2900 50  0001 C CIN
F 3 "http://www.icbank.com/data/ICBShop/board/HY1602E.pdf" H 6850 3900 50  0001 C CNN
	1    6650 3800
	1    0    0    -1  
$EndComp
$Comp
L Display_Character:HY1602E DS1
U 1 1 64FFAAF3
P 6650 1600
F 0 "DS1" H 6650 2581 50  0000 C CNN
F 1 "HY1602E" H 6650 2490 50  0000 C CNN
F 2 "Display:HY1602E" H 6650 700 50  0001 C CIN
F 3 "http://www.icbank.com/data/ICBShop/board/HY1602E.pdf" H 6850 1700 50  0001 C CNN
	1    6650 1600
	1    0    0    -1  
$EndComp
$Comp
L Regulator_Linear:L7805 U1
U 1 1 650040EB
P 1950 5050
F 0 "U1" H 1950 5292 50  0000 C CNN
F 1 "L7805" H 1950 5201 50  0000 C CNN
F 2 "" H 1975 4900 50  0001 L CIN
F 3 "http://www.st.com/content/ccc/resource/technical/document/datasheet/41/4f/b3/b0/12/d4/47/88/CD00000444.pdf/files/CD00000444.pdf/jcr:content/translations/en.CD00000444.pdf" H 1950 5000 50  0001 C CNN
	1    1950 5050
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C1
U 1 1 65005226
P 1400 5450
F 0 "C1" H 1518 5496 50  0000 L CNN
F 1 "57uF 16V" H 1518 5405 50  0000 L CNN
F 2 "" H 1438 5300 50  0001 C CNN
F 3 "~" H 1400 5450 50  0001 C CNN
	1    1400 5450
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C2
U 1 1 65005CE3
P 2400 5450
F 0 "C2" H 2518 5496 50  0000 L CNN
F 1 "57uF 16V" H 2518 5405 50  0000 L CNN
F 2 "" H 2438 5300 50  0001 C CNN
F 3 "~" H 2400 5450 50  0001 C CNN
	1    2400 5450
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 5050 1400 5050
Wire Wire Line
	1400 5050 1400 5300
Wire Wire Line
	2250 5050 2400 5050
Wire Wire Line
	2400 5050 2400 5300
Wire Wire Line
	1400 5600 1950 5600
Wire Wire Line
	1950 5350 1950 5600
Connection ~ 1950 5600
Wire Wire Line
	1950 5600 2400 5600
$Comp
L power:GND #PWR0101
U 1 1 6500A84A
P 1950 5650
F 0 "#PWR0101" H 1950 5400 50  0001 C CNN
F 1 "GND" H 1955 5477 50  0000 C CNN
F 2 "" H 1950 5650 50  0001 C CNN
F 3 "" H 1950 5650 50  0001 C CNN
	1    1950 5650
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0102
U 1 1 6500B208
P 3650 4250
F 0 "#PWR0102" H 3650 4000 50  0001 C CNN
F 1 "GND" H 3655 4077 50  0000 C CNN
F 2 "" H 3650 4250 50  0001 C CNN
F 3 "" H 3650 4250 50  0001 C CNN
	1    3650 4250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 6500BE42
P 7100 3950
F 0 "#PWR0103" H 7100 3700 50  0001 C CNN
F 1 "GND" H 7105 3777 50  0000 C CNN
F 2 "" H 7100 3950 50  0001 C CNN
F 3 "" H 7100 3950 50  0001 C CNN
	1    7100 3950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 6500CE1E
P 6650 2450
F 0 "#PWR0104" H 6650 2200 50  0001 C CNN
F 1 "GND" H 6655 2277 50  0000 C CNN
F 2 "" H 6650 2450 50  0001 C CNN
F 3 "" H 6650 2450 50  0001 C CNN
	1    6650 2450
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0105
U 1 1 6500DC8D
P 6650 4650
F 0 "#PWR0105" H 6650 4400 50  0001 C CNN
F 1 "GND" H 6655 4477 50  0000 C CNN
F 2 "" H 6650 4650 50  0001 C CNN
F 3 "" H 6650 4650 50  0001 C CNN
	1    6650 4650
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0106
U 1 1 6500E864
P 7100 1750
F 0 "#PWR0106" H 7100 1500 50  0001 C CNN
F 1 "GND" H 7105 1577 50  0000 C CNN
F 2 "" H 7100 1750 50  0001 C CNN
F 3 "" H 7100 1750 50  0001 C CNN
	1    7100 1750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0107
U 1 1 6500FDF7
P 6200 1200
F 0 "#PWR0107" H 6200 950 50  0001 C CNN
F 1 "GND" H 6205 1027 50  0000 C CNN
F 2 "" H 6200 1200 50  0001 C CNN
F 3 "" H 6200 1200 50  0001 C CNN
	1    6200 1200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0108
U 1 1 65010B69
P 6200 3400
F 0 "#PWR0108" H 6200 3150 50  0001 C CNN
F 1 "GND" H 6205 3227 50  0000 C CNN
F 2 "" H 6200 3400 50  0001 C CNN
F 3 "" H 6200 3400 50  0001 C CNN
	1    6200 3400
	1    0    0    -1  
$EndComp
Wire Wire Line
	6200 3400 6250 3400
Wire Wire Line
	7100 3950 7100 3900
Wire Wire Line
	7100 3900 7050 3900
Wire Wire Line
	6650 4650 6650 4600
Wire Wire Line
	6650 2450 6650 2400
Wire Wire Line
	7100 1750 7100 1700
Wire Wire Line
	7100 1700 7050 1700
Wire Wire Line
	6200 1200 6250 1200
Wire Wire Line
	3650 4250 3650 4200
Wire Wire Line
	3650 4200 3550 4200
Connection ~ 3650 4200
$Comp
L power:+9V #PWR0109
U 1 1 6501315B
P 1150 5050
F 0 "#PWR0109" H 1150 4900 50  0001 C CNN
F 1 "+9V" H 1165 5223 50  0000 C CNN
F 2 "" H 1150 5050 50  0001 C CNN
F 3 "" H 1150 5050 50  0001 C CNN
	1    1150 5050
	1    0    0    -1  
$EndComp
Wire Wire Line
	1150 5050 1400 5050
Connection ~ 1400 5050
$Comp
L power:+5V #PWR0110
U 1 1 65014419
P 2650 5050
F 0 "#PWR0110" H 2650 4900 50  0001 C CNN
F 1 "+5V" H 2665 5223 50  0000 C CNN
F 2 "" H 2650 5050 50  0001 C CNN
F 3 "" H 2650 5050 50  0001 C CNN
	1    2650 5050
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0111
U 1 1 650152D3
P 3750 2050
F 0 "#PWR0111" H 3750 1900 50  0001 C CNN
F 1 "+5V" H 3765 2223 50  0000 C CNN
F 2 "" H 3750 2050 50  0001 C CNN
F 3 "" H 3750 2050 50  0001 C CNN
	1    3750 2050
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0112
U 1 1 65015CCB
P 6650 750
F 0 "#PWR0112" H 6650 600 50  0001 C CNN
F 1 "+5V" H 6665 923 50  0000 C CNN
F 2 "" H 6650 750 50  0001 C CNN
F 3 "" H 6650 750 50  0001 C CNN
	1    6650 750 
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0113
U 1 1 65016C60
P 6650 2950
F 0 "#PWR0113" H 6650 2800 50  0001 C CNN
F 1 "+5V" H 6665 3123 50  0000 C CNN
F 2 "" H 6650 2950 50  0001 C CNN
F 3 "" H 6650 2950 50  0001 C CNN
	1    6650 2950
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0114
U 1 1 6501868D
P 7100 3800
F 0 "#PWR0114" H 7100 3650 50  0001 C CNN
F 1 "+5V" H 7115 3973 50  0000 C CNN
F 2 "" H 7100 3800 50  0001 C CNN
F 3 "" H 7100 3800 50  0001 C CNN
	1    7100 3800
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0115
U 1 1 650197D1
P 7100 1600
F 0 "#PWR0115" H 7100 1450 50  0001 C CNN
F 1 "+5V" H 7115 1773 50  0000 C CNN
F 2 "" H 7100 1600 50  0001 C CNN
F 3 "" H 7100 1600 50  0001 C CNN
	1    7100 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	7050 1600 7100 1600
Wire Wire Line
	7050 3800 7100 3800
Wire Wire Line
	3750 2200 3750 2050
Wire Wire Line
	2400 5050 2650 5050
Connection ~ 2400 5050
Wire Wire Line
	1200 950  1200 2800
Wire Wire Line
	1200 2800 3050 2800
Wire Wire Line
	1350 3600 3050 3600
Wire Wire Line
	3050 3900 1400 3900
Wire Wire Line
	1400 3900 1400 1150
Wire Wire Line
	1450 1250 1450 3700
Wire Wire Line
	1450 3700 3050 3700
Wire Wire Line
	3050 3800 1500 3800
Wire Wire Line
	1500 3800 1500 1350
Wire Wire Line
	4050 3400 5550 3400
Wire Wire Line
	5550 3400 5550 1300
Wire Wire Line
	5550 1300 6250 1300
Wire Wire Line
	6250 1000 5600 1000
Wire Wire Line
	5600 1000 5600 3500
Wire Wire Line
	5600 3500 4050 3500
Wire Wire Line
	6250 1900 5650 1900
Wire Wire Line
	5650 1900 5650 3600
Wire Wire Line
	5650 3600 4050 3600
Wire Wire Line
	6250 2000 2750 2000
Wire Wire Line
	2750 2000 2750 3500
Wire Wire Line
	2750 3500 3050 3500
Wire Wire Line
	6250 2100 2800 2100
Wire Wire Line
	2800 2100 2800 3400
Wire Wire Line
	2800 3400 3050 3400
Wire Wire Line
	6250 2200 5800 2200
Wire Wire Line
	5800 2200 5800 2150
Wire Wire Line
	2850 2150 2850 3300
Wire Wire Line
	2850 3300 3050 3300
Wire Wire Line
	3050 3200 3000 3200
Wire Wire Line
	3000 3200 3000 4550
Wire Wire Line
	3000 4550 6000 4550
Wire Wire Line
	6000 4550 6000 3500
Wire Wire Line
	6000 3500 6250 3500
Wire Wire Line
	6250 3200 5950 3200
Wire Wire Line
	5950 3200 5950 4600
Wire Wire Line
	5950 4600 2950 4600
Wire Wire Line
	2950 4600 2950 3100
Wire Wire Line
	2950 3100 3050 3100
Wire Wire Line
	3050 3000 2900 3000
Wire Wire Line
	2900 3000 2900 4650
Wire Wire Line
	2900 4650 6050 4650
Wire Wire Line
	6050 4650 6050 4100
Wire Wire Line
	6050 4100 6250 4100
Wire Wire Line
	6250 4200 6100 4200
Wire Wire Line
	6100 4200 6100 4700
Wire Wire Line
	6100 4700 2700 4700
Wire Wire Line
	2700 4700 2700 2900
Wire Wire Line
	2700 2900 3050 2900
Wire Wire Line
	3050 2700 2650 2700
Wire Wire Line
	2650 2700 2650 4750
Wire Wire Line
	2650 4750 6150 4750
Wire Wire Line
	6150 4750 6150 4400
Wire Wire Line
	6150 4400 6250 4400
Wire Wire Line
	6250 4300 6200 4300
Wire Wire Line
	6200 4300 6200 4800
Wire Wire Line
	6200 4800 2550 4800
Wire Wire Line
	2550 4800 2550 2600
Wire Wire Line
	2550 2600 3050 2600
Wire Wire Line
	6650 3000 6650 2950
Wire Wire Line
	6650 800  6650 750 
$Comp
L Device:R_POT_Dual RV1
U 1 1 6505801F
P 8350 1650
F 0 "RV1" H 8350 1417 50  0000 C CNN
F 1 "10k" H 8350 1326 50  0000 C CNN
F 2 "" H 8600 1575 50  0001 C CNN
F 3 "~" H 8600 1575 50  0001 C CNN
	1    8350 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 1750 8250 2150
Wire Wire Line
	8250 2150 8750 2150
Wire Wire Line
	8750 2150 8750 1750
Wire Wire Line
	7950 1750 7950 2050
Wire Wire Line
	7950 2050 8450 2050
Wire Wire Line
	8450 2050 8450 1750
$Comp
L power:+5V #PWR0116
U 1 1 6506215E
P 7850 2050
F 0 "#PWR0116" H 7850 1900 50  0001 C CNN
F 1 "+5V" H 7865 2223 50  0000 C CNN
F 2 "" H 7850 2050 50  0001 C CNN
F 3 "" H 7850 2050 50  0001 C CNN
	1    7850 2050
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0117
U 1 1 6506330B
P 8750 2150
F 0 "#PWR0117" H 8750 1900 50  0001 C CNN
F 1 "GND" H 8755 1977 50  0000 C CNN
F 2 "" H 8750 2150 50  0001 C CNN
F 3 "" H 8750 2150 50  0001 C CNN
	1    8750 2150
	1    0    0    -1  
$EndComp
Connection ~ 8750 2150
Wire Wire Line
	7850 2050 7950 2050
Connection ~ 7950 2050
Wire Wire Line
	8100 1550 8100 1000
Wire Wire Line
	8100 1000 7050 1000
Wire Wire Line
	8600 1550 8600 1100
Wire Wire Line
	8600 1100 7200 1100
Wire Wire Line
	7200 1100 7200 3200
Wire Wire Line
	7200 3200 7050 3200
$Comp
L Switch:SW_Push_Dual SW1
U 1 1 650732CD
P 4700 2450
F 0 "SW1" H 4700 2735 50  0000 C CNN
F 1 "SW_Push_Dual" H 4700 2644 50  0000 C CNN
F 2 "" H 4700 2650 50  0001 C CNN
F 3 "~" H 4700 2650 50  0001 C CNN
	1    4700 2450
	1    0    0    -1  
$EndComp
$Comp
L Device:R R1
U 1 1 65076A14
P 5200 2800
F 0 "R1" H 5270 2846 50  0000 L CNN
F 1 "1k" H 5270 2755 50  0000 L CNN
F 2 "" V 5130 2800 50  0001 C CNN
F 3 "~" H 5200 2800 50  0001 C CNN
	1    5200 2800
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 65078F00
P 5450 2800
F 0 "R2" H 5520 2846 50  0000 L CNN
F 1 "1k" H 5520 2755 50  0000 L CNN
F 2 "" V 5380 2800 50  0001 C CNN
F 3 "~" H 5450 2800 50  0001 C CNN
	1    5450 2800
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0118
U 1 1 6507A222
P 5200 2950
F 0 "#PWR0118" H 5200 2700 50  0001 C CNN
F 1 "GND" H 5205 2777 50  0000 C CNN
F 2 "" H 5200 2950 50  0001 C CNN
F 3 "" H 5200 2950 50  0001 C CNN
	1    5200 2950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0119
U 1 1 6507B5C3
P 5450 2950
F 0 "#PWR0119" H 5450 2700 50  0001 C CNN
F 1 "GND" H 5455 2777 50  0000 C CNN
F 2 "" H 5450 2950 50  0001 C CNN
F 3 "" H 5450 2950 50  0001 C CNN
	1    5450 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	4900 2650 4950 2650
Wire Wire Line
	5450 2450 5450 2650
Wire Wire Line
	4950 2650 4950 3200
Wire Wire Line
	4950 3200 4050 3200
Connection ~ 4950 2650
Wire Wire Line
	4950 2650 5200 2650
Wire Wire Line
	5050 2450 5050 3300
Wire Wire Line
	5050 3300 4050 3300
Connection ~ 5050 2450
Wire Wire Line
	5050 2450 5450 2450
Wire Wire Line
	4500 2650 4500 2450
Wire Wire Line
	4500 2450 4350 2450
Wire Wire Line
	4350 2450 4350 2200
Wire Wire Line
	4350 2200 3750 2200
Connection ~ 4500 2450
Connection ~ 3750 2200
$Comp
L Connector_Generic_Shielded:Conn_02x04_Odd_Even_Shielded J2
U 1 1 6509CA4E
P 2350 1050
F 0 "J2" H 2400 1365 50  0000 C CNN
F 1 "C1101" H 2400 1274 50  0000 C CNN
F 2 "" H 2350 1050 50  0001 C CNN
F 3 "~" H 2350 1050 50  0001 C CNN
	1    2350 1050
	1    0    0    -1  
$EndComp
$Comp
L Connector:Screw_Terminal_01x02 J1
U 1 1 6509E795
P 1050 4400
F 0 "J1" H 1130 4392 50  0000 L CNN
F 1 "9V Battery" H 1130 4301 50  0000 L CNN
F 2 "" H 1050 4400 50  0001 C CNN
F 3 "~" H 1050 4400 50  0001 C CNN
	1    1050 4400
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0120
U 1 1 650A2179
P 850 4500
F 0 "#PWR0120" H 850 4250 50  0001 C CNN
F 1 "GND" H 855 4327 50  0000 C CNN
F 2 "" H 850 4500 50  0001 C CNN
F 3 "" H 850 4500 50  0001 C CNN
	1    850  4500
	1    0    0    -1  
$EndComp
$Comp
L power:+9V #PWR0121
U 1 1 650A369A
P 850 4400
F 0 "#PWR0121" H 850 4250 50  0001 C CNN
F 1 "+9V" H 865 4573 50  0000 C CNN
F 2 "" H 850 4400 50  0001 C CNN
F 3 "" H 850 4400 50  0001 C CNN
	1    850  4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 1050 2100 1050
Wire Wire Line
	2100 1050 2100 950 
Wire Wire Line
	2150 1250 2150 1350
Wire Wire Line
	2050 1250 2050 1500
Wire Wire Line
	2050 1500 2800 1500
Wire Wire Line
	2800 1500 2800 1150
Wire Wire Line
	2800 1150 2650 1150
Wire Wire Line
	2850 1050 2850 1550
Wire Wire Line
	2850 1550 1350 1550
Wire Wire Line
	2650 1050 2850 1050
Wire Wire Line
	1350 1550 1350 3600
$Comp
L power:GND #PWR0122
U 1 1 6500F606
P 1650 750
F 0 "#PWR0122" H 1650 500 50  0001 C CNN
F 1 "GND" H 1655 577 50  0000 C CNN
F 2 "" H 1650 750 50  0001 C CNN
F 3 "" H 1650 750 50  0001 C CNN
	1    1650 750 
	1    0    0    -1  
$EndComp
Wire Wire Line
	2650 950  3650 950 
Wire Wire Line
	3650 950  3650 2200
Wire Wire Line
	2850 2150 5800 2150
Wire Wire Line
	1650 750  2150 750 
Wire Wire Line
	1200 950  2100 950 
Wire Wire Line
	1400 1150 2150 1150
Wire Wire Line
	1500 1350 2150 1350
Wire Wire Line
	1450 1250 2050 1250
Wire Wire Line
	2150 750  2150 950 
Wire Wire Line
	4900 2450 5050 2450
Connection ~ 4500 2650
Connection ~ 4900 2450
Connection ~ 4900 2650
$EndSCHEMATC
